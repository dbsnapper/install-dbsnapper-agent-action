...
function getUrl(version: string): string {
  ...
  const filename = `dbsnapper_${stripLeadingV(version)}_${ops}_${arch}`
  const extension = 'tar.gz'
  return `https://github.com/dbsnapper/dbsnapper/releases/download/v${stripLeadingV(
    version
  )}/${filename}.${extension}`
}
...
export async function getDownloadUrl(version?: string): Promise<string> {
  if (!!version && version !== 'latest') {
    core.info(`Downloading DBSnapper Agent version ${version}`)
...
async function getLatestVersion(): Promise<string> {
  try {
    const httpClient = new httpm.HttpClient('dbsnapper-install-gh-action', [], {
      headers: {
    ...
    const res = await httpClient.get(
      `https://api.github.com/repos/dbsnapper/dbsnapper/releases?per_page=1`
    )
    ...
    core.setFailed('Failed to resolve latest DBSnapper Agent version')
    throw err
  }
}
